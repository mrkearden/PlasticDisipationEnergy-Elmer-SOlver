...
real(kind=dp)::elstrain(6),plstrain(6)
  CHARACTER(LEN=1) :: c=','
!------------------------------------------------------------------------------
SAVE Visit, E,nu,hslope,yield,elstrain,plstrain
    ! Get Young's modulus and the Poisson ratio:
  IF (VISIT .EQ. 0) THEN
     open(unit=91,file='PDEOutput.csv',status='unknown')
     write(91,*) 'time',c,'noel',c,'npt',c,'ntens',c,'stress',c,'strain',c,'elstrain',c,'plstrain'
close(91)
visit = 1
end if
    E = Props(1)
...
do i=1,ntens
! write(*,*) j," Stress:",stress(j)," yield:",yield
...
elstrain = stress/E2
plstrain = stran-elstrain
do i =1,ntens
if (plstrain(i).lt.0.0) plstrain(i)=0.0
end do

do i=1,ntens
open(unit=91,file='PDEOutput.csv',status='unknown',position='append')
write(91,*) time(1),c,noel,c,npt,c,i,c,stress(i),c,stran(i),c,elstrain(i),c,plstrain(i)
close(91)
end do
